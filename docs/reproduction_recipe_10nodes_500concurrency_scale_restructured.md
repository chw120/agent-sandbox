...
# 3. Wait for helper pods to initialize, copy binary via kubectl cp, hot-swap binary, and restart both gcfsd & containerd:
for NODE in $NODES; do
  POD="patch-${NODE: -5}"
  (
    kubectl wait --for=condition=Ready pod/"$POD" -n kube-system --timeout=60s || true
    kubectl cp "$GCFSD_BIN" kube-system/"$POD":/host/home/kubernetes/bin/gcfsd-v2.new || true
    kubectl exec -n kube-system pod/"$POD" -- bash -c "
      set -ex
      PATH=/bin:/usr/bin:/sbin:/usr/sbin chroot /host systemctl stop gcfsd || true
      sleep 1
      cp /host/home/kubernetes/bin/gcfsd-v2 /host/home/kubernetes/bin/gcfsd-v2.bak 2>/dev/null || true
      ...
      PATH=/bin:/usr/bin:/sbin:/usr/sbin chroot /host systemctl daemon-reload || true
      PATH=/bin:/usr/bin:/sbin:/usr/sbin chroot /host systemctl restart gcfsd
      sleep 1
      # CRITICAL: Restart containerd so it re-establishes clean gRPC snapshotter connections to gcfsd
      # and purges any stale FUSE overlay mount descriptors (preventing CreateContainerError)
      PATH=/bin:/usr/bin:/sbin:/usr/sbin chroot /host systemctl restart containerd
      sleep 2
      PATH=/bin:/usr/bin:/sbin:/usr/sbin chroot /host curl -s -I http://127.0.0.1:11253/debug/pprof/ | head -n 2
    "
    kubectl delete pod "$POD" -n kube-system --grace-period=0 --force --ignore-not-found=true 2>/dev/null || true
  ) &
done
wait
echo "All 10 physical nodes patched with 6-CL VTProto driver and containerd restarted successfully!"
```
...
### 2.3 GCP Secondary Boot Disk (SBD) Image Creation (`gke-disk-image-builder` & Cross-Project Copy)
To provide a hardware-level pre-populated container image cache across all 10 nodes, we create a GCP Compute Engine Global Disk Image (`swebench-500-preload` or `swebench-baselayer-cache-v3-squashed-2`).

#### Method A: Instant Cross-Project Copy (Pre-Baked Official Image — Recommended)
Copy the official pre-baked 500-image dataset cache disk image directly from `gke-ai-eco-dev` into your GCP project:

```bash
gcloud compute images create swebench-500-preload \
  --project=$(gcloud config get-value project) \
  --source-image=swebench-500-preload \
  --source-image-project=gke-ai-eco-dev
```

#### Method B: Native GKE Image Builder (`gke-disk-image-builder` — From Scratch)
Alternatively, dynamically compute the majority-winner seed image and build a new global Secondary Boot Disk image from scratch using the Go builder tool:
...
# 2. Programmatically select the canonical public seed image containing this majority base layer:
SEED_IMAGE=$(cat examples/agent-sandbox-rl/swebench500_digests.txt | while read -r img; do if crane manifest "$img" | grep -q "$MAJORITY_DIGEST"; then echo "docker.io/$img"; break; fi; done)
echo "Selected majority seed image: $SEED_IMAGE"

# 3. CD into the tool directory (required for ./script/startup.sh template)
cd bin/ai-on-gke-tools/gke-disk-image-builder

# 4. Bake the global Secondary Boot Disk image using the dynamically selected seed image:
./gke-disk-image-builder \
  -project-name=$(gcloud config get-value project) \
  -zone=us-central1-a \
  -gcs-path=gs://$(gcloud config get-value project)-cache-bucket/logs \
  -image-name=swebench-baselayer-cache-v3-squashed-2 \
  -disk-size-gb=600 \
  -container-image=$SEED_IMAGE
```

---

### 2.4 Secondary Boot Disk 10-Node Super-Pool Creation (`--secondary-boot-disk`)
Once the SBD cache disk image is prepared, provision the 10-node `e2-standard-32` scale pool (**`10 x 96 vCPUs = 960 vCPUs / 3.84 TB RAM`**) configured with gVisor container sandboxing (`--sandbox=type=gvisor`), GKE's maximum architectural ceiling of **`--max-pods-per-node=256`**, and the Secondary Boot Disk attached in `CONTAINER_IMAGE_CACHE` mode:

```bash
# Provision the 10-node evaluation pool (960 vCPUs / 3.84TB RAM total) with SBD container image pre-population:
gcloud container node-pools create gvisor-scale-pool-32 \
  --cluster=agent-sandbox-scale-cluster \
  --zone=us-central1-c \
  --project=$(gcloud config get-value project) \
  ...
  --enable-image-streaming \
  --scopes="gke-default,storage-rw" \
  --secondary-boot-disk=disk-image=projects/$(gcloud config get-value project)/global/images/swebench-500-preload,mode=CONTAINER_IMAGE_CACHE \
  --quiet
```

*Note: In `COS_CONTAINERD` with `mode=CONTAINER_IMAGE_CACHE`, GKE automatically expands `/var/lib/containerd` on `/dev/sda1` to include the pre-loaded image snapshots, enabling zero-network cold-start pod creation.*
